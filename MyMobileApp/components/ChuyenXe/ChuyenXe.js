...
const ChuyenXe = ({ route }) => {
    ...
    const kiemTraNgayLe = (ngay) => {
        const ngayLe = ["2024-01-01", "2024-04-30", "2024-09-02"]; 
        const laNgayLe = ngayLe.includes(ngay);
        return laNgayLe;
    };

    const tinhGia = (ngay, gia) => {
        const laNgayLe = kiemTraNgayLe(ngay);
        if (laNgayLe) {
            const formatPrice = new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(gia*1.1);
            return formatPrice;
        } else {
